...
// A fixed reference time keeps the sample history identical on the server
// and the client. The registry version used Date.now() here, which is a
// hydration mismatch as soon as the component is server-rendered.
const FIXED_EPOCH = new Date("2026-01-01T12:00:00Z").getTime()
  ...
  const [tabs, setTabs] = useState<Tab[]>(() => {
    if (initialTabs && initialTabs.length > 0) {
      return initialTabs.map((tab, index) => ({
        // was Date.now() — non-deterministic ids break SSR hydration
        id: tab.id || `tab-${index}`,
    ...
    customHistory || [
      {
        ...
        timestamp: new Date(FIXED_EPOCH - 3600000),
        favicon: "🔍",
      },
      {
        id: "2",
        ...
        timestamp: new Date(FIXED_EPOCH - 7200000),
        favicon: "🐙",
      },
      {
        id: "3",
        ...
        timestamp: new Date(FIXED_EPOCH - 10800000),
        favicon: "📚",
      },
    ]
  )
